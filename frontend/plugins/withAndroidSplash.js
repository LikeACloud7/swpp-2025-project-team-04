const { withAndroidStyles, withDangerousMod, withPlugins } = require('@expo/config-plugins');
const fs = require('fs');
...
<layer-list xmlns:android="http://schemas.android.com/apk/res/android">
    ...
    <item>
        <bitmap
            ...
            fs.writeFileSync(path.join(resDir, 'launch_background.xml'), launchBackgroundXml);
            return config;
        },
    ]);
};
...
const withWindowBackgroundStyle = (config) => {
    return withAndroidStyles(config, async (config) => {
        ...
        const splashTheme = styles.resources.style.find((style) => style.$.name === 'Theme.App.SplashScreen');
        if (splashTheme) {
            if (splashTheme.item) {
                splashTheme.item = splashTheme.item.filter((item) => item.$.name !== 'android:windowBackground');
            } else {
                splashTheme.item = [];
            }
...
const withAndroidSplash = (config) => {
    return withPlugins(config, [withLaunchBackgroundDrawable, withWindowBackgroundStyle]);
};
